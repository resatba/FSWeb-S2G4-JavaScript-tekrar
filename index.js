...
const tekrarEdenElemanlar = {};
    
sayilar.forEach(eleman => {
	if (tekrarEdenElemanlar[eleman]) {
		tekrarEdenElemanlar[eleman]++;
	} else {
		tekrarEdenElemanlar[eleman] = 1;
	}
});

const sonuclar = {};
for (const eleman in tekrarEdenElemanlar) {
	if (tekrarEdenElemanlar[eleman] > 1) {
		sonuclar[eleman] = tekrarEdenElemanlar[eleman];
	}
}

tekraredensayilar = []
for (const sonuc in sonuclar) {
	tekraredensayilar.push(`${sonuc} sayısı ${sonuclar[sonuc]} kere tekrar edilmiştir`)
}
